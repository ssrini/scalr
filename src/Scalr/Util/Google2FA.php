...
class Scalr_Util_Google2FA
{
	...
	public static function oathHotp($key, $counter)
	{
		...
		$binCounter = pack('N*', 0) . pack('N*', $counter); // Counter must be 64-bit int
	 ...
	 * @return boolean
	 **/
	public static function verifyKey($b32seed, $key, $offset = -2, $window = 4)
	{
		$timeStamp = self::getTimestamp();
		$binarySeed = self::base32Decode($b32seed);

		for ($ts = $timeStamp + $offset; $ts <= $timeStamp + $window; $ts++)
