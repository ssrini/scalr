Scalr.regPage('Scalr.ui.farms.monitoring.view', function (loadParams, moduleParams) {
	...
	function fillStatistic(farm, watchername, type, role){
		if(panel.down('#' + watchername + farm + role).body) panel.down('#' + watchername + farm + role).body.update('<div style="position: relative; top: 48%; text-align: center; width: 100%; height: 50%;"><img src = "/ui/images/icons/anim/loading_16x16.gif">&nbsp;Loading...</div>');
		panel.down('#' + watchername + farm + role).html = '<div style="position: relative; top: 48%; text-align: center; width: 100%; height: 50%;"><img src = "/ui/images/icons/anim/loading_16x16.gif">&nbsp;Loading...</div>';
		Scalr.Request({
			scope: this,
