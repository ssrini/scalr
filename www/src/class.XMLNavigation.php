<?
	...
	class XMLNavigation extends Core
	{
		...
		public function AddNode($DOMPart, &$menu_part)
		{
            foreach ($DOMPart->childNodes as $node)
            {
                if (!($node instanceof DOMText))
                {
                    ...
                    {
                        $entries = @$this->XPath->query('node[@title = "'.$node->getAttribute("title").'"]', $menu_part);
		...
		*/
		...
		{
			if (!is_readable($path))
				self::RaiseError(_("failed to read nav.xml"));
		 ...
		 * @return string
		 */
		public function GetBreadCrumbs($filename = false)
		{
		    if (!$filename)
            ...
            {
                array_push($items, array("href"=>"", "name"=>$entry->nodeValue));
                
                while ($entry->parentNode->nodeName != 'menu')
                {
                    if ($entry->parentNode->nodeName == 'node')
                    {
                        if (!$entry->parentNode->hasAttribute("href"))
                        {
                            $item = $xpath->query("item", $entry->parentNode)->item(0);
                        ...
                        {
                            $href = $entry->parentNode->getAttribute("href");
                            if ($href == "")
                                $href = false;
                        }
                        
                        array_push($items, array("href"=>$href, "name"=>$entry->parentNode->getAttribute("title")));
            ...
            {
                if ($item["href"])
                    array_push($bcrumbs, "<a href='{$item['href']}'>{$item['name']}</a>");
                else 
                    array_push($bcrumbs, "{$item['name']}");
            }
            
            return implode("&nbsp;&raquo;&nbsp;", $bcrumbs);
		}
